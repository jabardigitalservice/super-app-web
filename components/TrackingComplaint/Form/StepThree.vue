<template>
  <section class="grid grid-cols-1 gap-x-8 gap-y-6 mb-4">
    <h3 class="font-roboto font-medium text-black text-[14px] leading-[20px]">
      ...
      <span class="font-lato text-[12px] leading-[18px] text-gray-600 text-left">
    ...
    <transition-group name="slide-fade">
      <div v-for="(file) in imagesFile" :key="file.id" class="flex flex-col gap-2 mb-4">
        ...
        />
        <span v-show="!!file.errors" class="font-lato text-[13px] text-red-700">{{ file.errors[0] }}</span>
      </div>
    </transition-group>
  </section>
</template>
