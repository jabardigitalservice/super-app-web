...
export default {
  ...
  getters: {
    ...
    isLastStep(state) {
      return state.currentFormStep === 5
    },
  ...
  mutations: {
    ...
    SET_DATA_WARGI_KK(state, payload) {
      state.data_wargi.kk = payload
    },
    SET_PERHATIAN_PRIVACY_AGREED(state, payload) {
      state.perhatian.is_privacy_agreed = payload
    },
    SET_PERHATIAN_BENEFICIARY(state, payload) {
      state.perhatian.is_beneficiary = payload
    },
  ...
  actions: {
    ...
    async fetchUserProfile({ state, commit, dispatch }) {
      try {
        ...
        commit('SET_DATA_WARGI_EMAIL', email)
      } catch (error) {
    ...
    async handleImagesFile({ state, getters, dispatch }, files) {
      for (const file of files) {
        ...
        if (errorMessages.length > 0) {
          ...
          state.foto_aduan.raw_image[indexing].errors = errorMessages
        } else {
          state.foto_aduan.raw_image[indexing].image_upload_status =
            IMAGE_UPLOAD_STATUS.SUCCESS
          state.foto_aduan.raw_image[indexing].errors = []
        }

        // Force reactivity update
        state.foto_aduan.raw_image = [...state.foto_aduan.raw_image]
      }
    },
    ...
    async retryHandleImage({ state, dispatch }, file) {
      ...
      if (errorMessages.length > 0) {
        state.foto_aduan.raw_image[itemIndex].image_upload_status =
          IMAGE_UPLOAD_STATUS.ERROR
        state.foto_aduan.raw_image[itemIndex].errors = errorMessages
      } else {
        state.foto_aduan.raw_image[itemIndex].image_upload_status =
          IMAGE_UPLOAD_STATUS.SUCCESS
        state.foto_aduan.raw_image[itemIndex].errors = []
      }
